...
def train(
    ...
    for step in tqdm(range(main_num_steps)):
        ...
        #---------------------------------------------------------------#
        logit_b, features_mid_b = model_b(data)
        layer1_b = features_mid_b[0]
        layer2_b = features_mid_b[1]
        # layer3_b = features_mid_b[5]
        #---------------------------------------------------------------#
        ...
        #---------------------------------------------------------------#
        logit_d, features_mid_d = model_d(data)
        layer1_d = features_mid_d[0]
        layer2_d = features_mid_d[1]
        # layer3_d = features_mid_d[5]
        #---------------------------------------------------------------#
        ...
        # Forward KL
        loss_kl_1 = 1 - kl_loss(layer1_b.softmax(1),layer1_d.softmax(1))
        loss_kl_2 = 1 - kl_loss(layer2_b.softmax(1),layer2_d.softmax(1))
        # loss_kl_2 = kl_loss(layer2_b,layer2_d)
        # loss_kl_3 = kl_loss(layer3_b,layer3_d)
        # # Reverse KL
        # loss_kl_1 = kl_loss(layer1_d,layer1_b)
        # loss_kl_2 = kl_loss(layer2_d,layer2_b)
        # loss_kl_3 = kl_loss(layer3_d,layer3_b)

#--------------------------------------------------------------------------------------------#

        loss = loss_b_update.mean() + loss_d_update.mean() + lambda_ortho* (loss_kl_1 + loss_kl_2 ) 
#--------------------------------------------------------------------------------------------#
        ...
        layer_names = ['l2.weight', 'l3.weight']
        ortho_loss = calculate_cosine_similarity_loss(model_d, model_b, layer_names)
        loss2 = (0.005) * ortho_loss
        loss2 = loss2.mean()
        optimizer_d.zero_grad()
        logit_d, _ = model_d(data)
        loss2.backward()
        optimizer_d.step()
    ...
    model_b_file_name = f"model_f_kl_{lambda_ortho}_b__02.th"
    model_d_file_name = f"model_f_kl_{lambda_ortho}_d__02.th"
