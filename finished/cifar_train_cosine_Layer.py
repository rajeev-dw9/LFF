...
#--------------------------------------------------------------------------------------------#
        ...
        # layer_names = ["layer1.1.conv2.weight","layer2.1.conv2.weight"]
        # ortho_loss = calculate_cos_sim(model_d, model_b, [2,4])
        #ortho_loss = calculate_cosine_similarity_loss(model_d, model_b, layer_names=layer_names)
...
#--------------------------------------------------------------------------------------------#
        ...
        # ortho_loss = calculate_cosine_similarity_loss(model_d, model_b, layer_names=layer_names)
        layer_names_a = ['feature.2.weight']
        layer_names_b = ['feature.4.weight']
        ortho_loss_a = calculate_cosine_similarity_loss(model_d, model_b, layer_names=layer_names_a)
        ortho_loss_b = calculate_cosine_similarity_loss(model_d, model_b, layer_names=layer_names_b)
        loss2 = (lambda_ortho / 2) * (ortho_loss_a + ortho_loss_b)
        loss2 = loss2.mean()
        optimizer_d.zero_grad()
        loss2.backward()
        optimizer_d.step()
#--------------------------------------------------------------------------------------------#
    ...
    result_file_name = f"cifar_result_cosine__{lambda_ortho}__mmmmmm.th"
    model_file_name = f"cifar_model_cosine__{lambda_ortho}__mmmmmm.th"
