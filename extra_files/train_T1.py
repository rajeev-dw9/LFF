        ...
        if 'weight' in name1:
            for layer_name in layer_names:
                ...
                if layer_name in name1:
                    ...
                    break  # Break the inner loop after finding a matching layer_name
    return cosine_loss
...
#--------------------------------------------------------------------------------------------#
        ...
        ortho_loss_a = calculate_cosine_similarity_loss(model_d, model_b, layer_names_a)
        ortho_loss_b = calculate_cosine_similarity_loss(model_d, model_b, layer_names_b)
        loss = loss_b_update.mean() + loss_d_update.mean() + (lambda_ortho / 2) * (ortho_loss_a + ortho_loss_b)
#--------------------------------------------------------------------------------------------#
    ...
    model_b_file_name = f"model_b_cosine__neg__{lambda_ortho}__t1.th"
    model_d_file_name = f"model_d_cosine__neg__{lambda_ortho}__t1.th"
