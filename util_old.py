...
class MultiDimAverageMeter(object):
    def __init__(self, dims):
        self.dims = dims
        self.cum = torch.zeros(np.prod(dims)).cuda()
        self.cnt = torch.zeros(np.prod(dims)).cuda()
    ...
    def add(self, vals, idxs):
        ...
        self.cum.index_add_(0, flattened_idx, vals.view(-1).float().cuda())
        self.cnt.index_add_(
            0, flattened_idx, torch.ones_like(vals.view(-1), dtype=torch.float).cuda()
        )
...
class EMA:
    ...
    def max_loss(self, label):
        label_index = np.where(self.label.cpu() == label)[0]
